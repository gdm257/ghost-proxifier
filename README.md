...
Ghost Proxifier 使用 **HTTP CONNECT** 方法与上游代理通信：
...
在 Hook 场景下，`connect()` 拿到的是 **IP 地址**而非域名。Ghost Proxifier 通过 DNS 解析时建立的 `IP → 域名` 映射表反查，**优先使用域名**发送 CONNECT 请求，让上游代理能基于域名做精确分流。

当反查不到域名时，回退为使用 IP 发送 CONNECT，此时上游代理只能依赖 **GeoIP 规则** 判断流量走向。如果 DNS 被污染返回了错误的 IP，GeoIP 判定也会跟着出错。这正是 **Local DNS 防污染**至关重要的原因。
...
同时记录 `IP → 域名` 映射表，确保 GeoIP 分流规则正常工作。

### 4. DoH（DNS-over-HTTPS）阻断

Chrome 等浏览器会尝试使用 DNS-over-HTTPS（DoH），直接通过 HTTPS 查询 DNS，绕过 Local DNS Proxy。

Ghost Proxifier 识别已知 DoH 服务器 IP（如 `8.8.8.8:443`、`1.1.1.1:443`）并在 `connect()` 阶段直接返回 `WSAECONNREFUSED`，强制浏览器回退到标准 DNS。
...
由于项目依赖 MinHook 子模块，克隆时请加上 `--recursive`：
